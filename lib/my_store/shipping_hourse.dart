...
class ShippingHoursePage extends StatefulWidget {
  final String type;
  ShippingHoursePage({this.type});
  @override
  _ShippingHoursePageState createState() => _ShippingHoursePageState();
}

class _ShippingHoursePageState extends State<ShippingHoursePage> {
  List shopList = [];
  String freightId = "";
  @override
  void initState() {
  ...
  void getList() {
    ...
    StoreServer().getShippingHourseList(map, (success) async {
      setState(() {
  ...
  void delete() {
    ...
    StoreServer().delShippingHourse(map, (success) async {
      ...
      var slide = SlideButton(
        ...
        buttons: <Widget>[
          buildAction(key, "编辑", Colors.amber, () {
            ...
            NavigatorUtils.goAddShippingHoursePage(context, shopList[i])
                .then((res) {
              getList();
            });
          }),
  ...
  Widget build(BuildContext context) {
    ...
    Widget btnArea = new Container(
      ...
      child: BigButton(
        name: '+新增发货仓',
        tapFun: () {
          NavigatorUtils.goAddShippingHoursePage(context).then((res) {
            getList();
          });
        },
