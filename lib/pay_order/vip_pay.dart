...
class VipFukuanWidget extends StatefulWidget {

  final VipRecharge vipRecharge;
...
class DialogContentState extends State<VipFukuanWidget> {
  int groupValue = 1;
  ...
  void payOrder() async {
    ...
    map.putIfAbsent("amount", () => widget.vipRecharge.total);
     map.putIfAbsent("day", () => widget.vipRecharge.time);
    map.putIfAbsent("level", () => widget.vipRecharge.level);
    map.putIfAbsent("pay_state", () => groupValue);
    GoodsServer().buyVip(map, (success) async {
      setState(() {
        isloading = false;
      });
